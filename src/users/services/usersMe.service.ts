  ...
  async findOne(req: any): Promise<CreateUserDto> {
    ...
    try {
      ...
      const user = await this.usersModule
        ...
        .select(
          '_id chatSocketId chatSocketAdminId firstName lastName avatar email userName role age phoneNumber active gender nationality country passportNumber',
        );
  ...
  async update(req: any, updateUserDto: UpdateUserDto): Promise<any> {
    ...
    try {
      ...
      const user = await this.usersModule
        .findOne({ _id: payload._id })
        .select(
          ' _id chatSocketId chatSocketAdminId firstName lastName avatar email userName role age phoneNumber active updatedAt',
        );
